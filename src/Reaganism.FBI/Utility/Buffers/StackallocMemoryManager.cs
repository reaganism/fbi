...
namespace Reaganism.FBI.Utility.Buffers;
...
/// </remarks>
internal sealed unsafe class StackallocMemoryManager<T> : MemoryManager<T> where T : unmanaged
{
    private readonly T*  ptr;
    private readonly int len;

    public StackallocMemoryManager(Span<T> span)
    {
        ptr = (T*)Unsafe.AsPointer(ref MemoryMarshal.GetReference(span));
        len = span.Length;
    }

    public StackallocMemoryManager(T* ptr, int len)
    {
        this.ptr = ptr;
        this.len = len;
    }
