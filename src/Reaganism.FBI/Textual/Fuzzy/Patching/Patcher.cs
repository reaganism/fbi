...
namespace Reaganism.FBI.Textual.Fuzzy.Patching;
...
public sealed class Patcher
{
    ...
    [PublicAPI]
    public sealed class Result(FuzzyPatch patch)
    {
        [PublicAPI]
        public FuzzyPatch Patch { [PublicAPI] get; } = patch;
        ...
        [PublicAPI]
        public FuzzyPatch? AppliedPatch { [PublicAPI] get; [PublicAPI] set; }
        ...
        {
            var header = FuzzyPatchHeader.GetHeader(Patch, false);
    ...
    private sealed class WorkingPatch(FuzzyPatch patch)
    {
        public FuzzyPatch Patch { get; } = patch;
        ...
        public void Succeed(Mode mode, FuzzyPatch appliedPatch)
        {
        ...
        public void LinesToIds(FuzzyTokenMapper tokenMapper)
        {
            LmContext = new string(tokenMapper.LinesToIds(Patch.ContextLines).Select(x => (char)x).ToArray());
            LmPatched = new string(tokenMapper.LinesToIds(Patch.PatchedLines).Select(x => (char)x).ToArray());
        }

        public void WordsToIds(FuzzyTokenMapper tokenMapper)
        {
    ...
    private struct MatchRunner(int loc, int dir, FuzzyMatchMatrix[] mms, float penaltyPerLine)
    {
        private int       loc    = loc;
    ...
    {
        [PublicAPI]
        public int MaxMatchOffset { [PublicAPI] get; [PublicAPI] set; } = FuzzyMatchMatrix.DEFAULT_MAX_OFFSET;
    ...
    private readonly FuzzyTokenMapper            tokenMapper;
    ...
    private FuzzyPatch? lastAppliedPatch;
    ...
    [PublicAPI]
    public Patcher(IEnumerable<FuzzyPatch> patches, IEnumerable<string> lines, FuzzyTokenMapper? tokenMapper = null)
    {
        this.patches     = patches.Select(x => new WorkingPatch(x)).ToList();
        this.lines       = lines.ToList();
        this.tokenMapper = tokenMapper ?? new FuzzyTokenMapper();
    }
    ...
    private void LinesToIds()
    {
        ...
        lmText = new string(tokenMapper.LinesToIds(lines).Select(x => (char)x).ToArray());
    }
    ...
    private FuzzyPatch ApplyExactAt(int loc, WorkingPatch patch)
    {
        if (!patch.Patch.ContextLines.SequenceEqual(lines.GetRange(loc, patch.Patch.Range1.Length)))
        {
            var linesA = patch.Patch.ContextLines;
            var linesB = lines.GetRange(loc, patch.Patch.Range1.Length);
            // TODO: bro what
        ...
        {
            appliedPatch = new FuzzyPatch(patch.Patch)
            {
    ...
    private bool CanApplySafelyAt(int loc, FuzzyPatch patch)
    {
        if (loc >= ModifiedRange.End)
        {
            return true;
    ...
    private bool ApplyOffset(WorkingPatch patch)
    {
        ...
        var reverse = lmText.LastIndexOf(patch.LmContext!, Math.Min(loc + patch.LmContext!.Length, lines.Count - 1));
    ...
    private static (int[]? match, float score) FuzzyMatch(IReadOnlyList<string> wmPattern, IReadOnlyList<string> wmText, int loc, FuzzyMatchOptions? options = null, LineRange[]? ranges = null)
    {
        ...
        var fwdMatchers = ranges.Select(x => new FuzzyMatchMatrix(wmPattern,           wmText, options.MaxMatchOffset, x)).SkipWhile(x => loc > x.WorkingRange.Last).ToArray();
        var revMatches  = ranges.Reverse().Select(x => new FuzzyMatchMatrix(wmPattern, wmText, options.MaxMatchOffset, x)).SkipWhile(x => loc < x.WorkingRange.First).ToArray();
    ...
    private static FuzzyPatch AdjustPatchToMatchedLines(FuzzyPatch patch, int[] match, IReadOnlyList<string> lines)
    {
        var fuzzyPatch = new FuzzyPatch(patch);
        ...
        for (int i = 0, j = 0, pLoc = -1; i < patch.Range1.Length; i++)
        {
            ...
            if (mLoc >= 0 && pLoc >= 0 && mLoc - pLoc > 1)
            {
                ...
                var op = diffs[j - 1].Operation == FuzzyOperation.DELETE && diffs[j].Operation == FuzzyOperation.DELETE ? FuzzyOperation.DELETE : FuzzyOperation.EQUALS;

                for (var l = pLoc + 1; l < mLoc; l++)
                {
                    diffs.Insert(j++, new FuzzyDiffLine(op, Utf16String.FromString(lines[l])));
                }
            }
            ...
            while (diffs[j].Operation == FuzzyOperation.INSERT)
            {
                j++;
            }
            ...
            else
            {
                ...
                // doesn't matter).
                diffs[j] = new FuzzyDiffLine(diffs[j].Operation, Utf16String.FromString(lines[mLoc]));
                j++;
            }
        }

        fuzzyPatch.RecalculateLengths();
        return fuzzyPatch;
    }
